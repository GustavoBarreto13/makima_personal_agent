// icons.tsx — Mapa de ícones SVG da Komi + componentes Icon e Avatar.
// Portado diretamente do icons.jsx do design handoff.
// Todos os paths são strings SVG comprimidas (sem <svg> externo — o componente Icon gera o <svg>).

import React from 'react'
import { avatarColor, initials } from './lib'

// ─── Mapa de paths SVG ────────────────────────────────────────────────────
// Cada entrada é um path SVG sem o prefixo "M" inicial — o Icon adiciona "M" antes.
// Formato: chave = nome do ícone, valor = string(s) de path separadas por "M".
...
/**
 * Renderiza um ícone SVG a partir do mapa ICONS.
 * Se o nome não existir, cai no ícone "user" como fallback.
 * Todos os paths são stroke (sem fill) com strokeWidth 1.8.
 */
export function Icon({ name, style, className }: IconProps) {
  // Usa o path do mapa ou cai no "user" como fallback
  ...
  return (
    ...
    >
      {/* Divide o path pelas ocorrências de "M" — cada segmento vira um <path> */}
...
/**
 * Exibe a foto do avatar se disponível, ou as iniciais com cor determinística.
 * A cor de fundo das iniciais é sempre a mesma para o mesmo nome (hash estável).
 * Tamanhos disponíveis: 28, 40, 48, 96 (pixels — controlados pelo CSS .avatar.a-N).
 */
export function Avatar({ person, size = 40 }: AvatarProps) {
  // Classe CSS combina o estilo base com o modificador de tamanho
  const cls = `avatar a-${size}`

  if (person.avatar_url) {
    // Foto disponível: exibe a imagem (object-fit: cover via CSS)
    return (
      <div className={cls}>
  ...
  // Sem foto: exibe as iniciais com cor de fundo determinística
  return (
