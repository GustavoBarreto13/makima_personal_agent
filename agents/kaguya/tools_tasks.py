...
def _gcal_events_for_day(day_str: str) -> tuple[list[dict], list[tuple[int, int]], list[tuple[int, int]], list[tuple[int, int]], bool]:
    ...
    try:
        ...
        # Busca os eventos do Google. exclude explícito (spec 067/069): além do
        # padrão (_DEFAULT_EXCLUDE — espelho Kaguya + TickTick + calendários-espelho
        # do hub), esconde também "Kaguya — Hábitos" — os alertas de hábito aparecem
        # na tela de Calendário (que usa o exclude PADRÃO, sem habitos), mas não
        # devem inflar a capacidade nem a timeline do Meu Dia (hábito só entra aqui
        # via seleção explícita — ver a chave "habitos").
        raw_events = _gcal.list_events(
            day_str, day_str, exclude=(*_gcal._DEFAULT_EXCLUDE, "Kaguya — Hábitos")
        )
