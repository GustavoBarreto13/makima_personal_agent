...
function TaskModal({ task, onSave, onClose, onDelete }) {
  ...
  const [f, setF] = React.useState(() => ({
    ...
    tags: [...(task.tags || [])], subtasks: (task.subtasks || []).map(s => ({ prio: 0, notes: '', ...s })),
  ...
  const addSub = () => set('subtasks', [...f.subtasks, { id: 's' + Date.now(), title: '', done: false, prio: 0, notes: '' }]);
  const patchSub = (i, patch) => set('subtasks', f.subtasks.map((s, j) => j === i ? { ...s, ...patch } : s));
  const toggleSub = (i) => patchSub(i, { done: !f.subtasks[i].done });
  const delSub = (i) => set('subtasks', f.subtasks.filter((_, j) => j !== i));
  ...
  const save = () => {
    ...
    onSave({
      ...
      tags: f.tags, subtasks: f.subtasks.filter(s => s.title.trim()), recur: f.recur,
  ...
  return (
    <Scrim onClose={onClose} maxWidth={540}>
      ...
      <div className="modal-body">
        ...
        <div className="modal-field">
          <label className="modal-label">Subtarefas</label>
          <div className="subtask-list">
            {f.subtasks.map((s, i) => (
              <div key={s.id} className="subtask-card">
                <div className="subtask-row">
                  <Check done={s.done} onClick={() => toggleSub(i)} size={16} />
                  <input className={s.done ? 'done-text' : ''} value={s.title} placeholder="Subtarefa…"
                         onChange={(e) => patchSub(i, { title: e.target.value })} />
                  <button className="sub-prio" title={'Prioridade: ' + PRIO[s.prio || 0].name}
                          style={{ color: PRIO[s.prio || 0].color || 'var(--ink-4)' }}
                          onClick={() => patchSub(i, { prio: ((s.prio || 0) + 1) % 4 })}>
                    <Icon name="flag" style={{ width: 13, height: 13 }} />
                  </button>
                  <button className="modal-x" style={{ width: 24, height: 24 }} onClick={() => delSub(i)}><Icon name="x" /></button>
                </div>
                <input className="subtask-notes" value={s.notes || ''} placeholder="descrição (opcional)…"
                       onChange={(e) => patchSub(i, { notes: e.target.value })} />
              </div>
            ))}
            <div className="subtask-add" onClick={addSub}><Icon name="plus" /> Adicionar subtarefa</div>
          </div>
        </div>
