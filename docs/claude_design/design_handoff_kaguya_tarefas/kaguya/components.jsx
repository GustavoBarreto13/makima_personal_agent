  ...
  return (
    <div className={'kcard' + (dragging ? ' dragging' : '')} draggable
      ...
      <div className="kcard-meta">
        {task.prio > 0 && <PrioFlag level={task.prio} />}
        {task.due && <DateChip iso={task.due} time={task.time} done={task.done} />}
        {task.tags.slice(0, 1).map(t => <TagChip key={t} tag={t} />)}
        {subs.length > 0 && <span className="tk-est"><Icon name="list" style={{ width: 11, height: 11, display: 'inline', verticalAlign: -1 }} /> {subs.filter(s => s.done).length}/{subs.length}</span>}
        <span className="tk-est" style={{ marginLeft: 'auto' }}><span className="cp-dot" style={{ display: 'inline-block', width: 7, height: 7, borderRadius: 3, background: projById(task.project).color, marginRight: 4, verticalAlign: 0 }} />{projById(task.project).name}</span>
      </div>
    </div>
  );
}
