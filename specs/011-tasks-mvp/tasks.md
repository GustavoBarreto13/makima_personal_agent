...
- [ ] T004 [P] Criar `agents/kaguya/tools_tasks.py` sobre `agents/db.py`: `list_tasks`, `list_tasks_today` (shape `{overdue, today}` por projeto), `search_tasks`, `list_trash`, `create_task` (default Inbox, `parent_id` 1 nível), `update_task` (regra de coluna ao trocar projeto), `complete_task(cascade)` (erro `needs_cascade` com subtarefas abertas), `reopen_task` (bloqueia pai concluído), `reorder_task` (posição esparsa + renormalização transacional), `delete_task`/`restore_task` (soft delete) — validações e mensagens de erro em português (regras: `data-model.md`)
...
- [ ] T008 [P] [US1] Criar fundação do shell: `webapp/frontend/src/pages/kaguya/kaguya.css` (tokens OKLCH escopados `.kg-app`, temas light/dark, identidade rosa — seguir `specs/010-kaguya-tasks-app/frontend-design-guide.md` §2), `types.ts` (espelhando `contracts/api-tasks.md`) e `kaguyaApi.ts` sobre `lib/api.ts`
- [ ] T009 [US1] Criar `webapp/frontend/src/pages/kaguya/KaguyaShell.tsx` (navegação interna por estado `{view, param}`), `components/SidebarNav.tsx` (grupos → projetos com contagem, Inbox no topo, views fixas) e `TweaksPanel.tsx` (tema/acento/densidade, padrão dos outros shells)
...
- [ ] T011 [US1] Criar `screens/ListScreen.tsx` + `components/TaskRow.tsx`: lista por `position`, subtarefas aninhadas colapsáveis, checkbox de conclusão (confirmação de cascata), edição inline do título, drag-and-drop chamando `POST /position`
- [ ] T012 [P] [US1] Criar `modals/TaskModal.tsx` e `modals/ProjectModal.tsx` (portar o padrão `FieldDef`/`FormModal` da Nami — copiar e adaptar, não importar cross-shell): campos título/descrição/projeto/prioridade/data/hora; exclusão de projeto com escolha mover-para-Inbox vs excluir
...
- [ ] T024 [P] [US4] Criar `components/QuickAdd.tsx`: parsing determinístico no frontend (research D6) — `#projeto` (prefixo case-insensitive contra a sidebar carregada) e `!alta|!media|!baixa` viram chips destacados em tempo real (padrão visual `RichText` do Violet); token não resolvido permanece no título; `#inexistente` → Inbox com aviso
