    ...
    <div className={'kcard' + (dragging ? ' dragging' : '') + (task.done ? ' done' : '')} draggable
      ...
      <div className="kcard-body">
        ...
        <div className="kcard-meta">
          {(task.assignees || []).length > 0 && <AvatarStack ids={task.assignees} size={18} />}
          {task.due && <DateChip iso={task.due} time={task.time} done={task.done} />}
      ...
      {task.done
        ...
        : prog.total > 0
          ? <div className="kcard-ring"><ProgressRing pct={prog.done / prog.total} /><span className="kr-lbl">{prog.done}/{prog.total}</span></div>
          : null}
    </div>
  );
}
