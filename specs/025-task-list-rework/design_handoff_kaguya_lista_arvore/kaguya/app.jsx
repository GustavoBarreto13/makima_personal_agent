...
const CAL_VARIANT = { 'Ágora': 'agora', 'Google': 'helvetico', 'Editorial': 'editorial' };
...
function App() {
  ...
  // hub: ?novo=1 abre nova tarefa direto
  React.useEffect(() => { if (new URLSearchParams(window.location.search).get('novo') === '1') onNew({ project: 'inbox' }); }, []);
  ...
  const onToggleSub = (task, subId) => { const tk = TASKS.find(x => x.id === subId); if (tk) { tk.done = !tk.done; tk.col = tk.done ? 'done' : 'todo'; refresh(); } };

  /* ── API da árvore de tarefas (estrutura global de subtarefas) ── */
  const maxRootPos = () => { const r = TASKS.filter(x => !x.parent); return r.length ? Math.max(...r.map(x => x.pos)) : 0; };
  const treeApi = {
    refresh,
    create: (obj) => { TASKS.push(obj); refresh(); },
    setDone: (task, done) => { const tk = TASKS.find(x => x.id === task.id); if (tk) { tk.done = done; tk.col = done ? 'done' : 'todo'; refresh(); } },
    rename: (task, title) => onUpdate(task.id, { title }),
    setAssignees: (task, ids) => onUpdate(task.id, { assignees: ids }),
    remove: (task) => { const ids = new Set([task.id, ...descendantsOf(task.id).map(d => d.id)]); for (let i = TASKS.length - 1; i >= 0; i--) if (ids.has(TASKS[i].id)) TASKS.splice(i, 1); refresh(); },
    addRoot: (project, setEditingId) => { const t = mk({ project: project || 'inbox', col: 'todo', parent: null, pos: maxRootPos() + 1000 }); TASKS.push(t); refresh(); setEditingId && setEditingId(t.id); },
    addChild: (parent, setEditingId, openParent) => {
      openParent && openParent();
      const kids = childrenOf(parent.id);
      const pos = (kids.length ? Math.max(...kids.map(k => k.pos)) : parent.pos) + 10;
      const t = mk({ parent: parent.id, project: parent.project, col: parent.col, pos });
      TASKS.push(t); refresh(); setEditingId && setEditingId(t.id);
    },
    addSibling: (sib, setEditingId) => { const t = mk({ parent: sib.parent, project: sib.project, col: sib.col, pos: sib.pos + 5 }); TASKS.push(t); refresh(); setEditingId && setEditingId(t.id); },
    indent: (task) => {
      const sibs = TASKS.filter(x => x.parent === task.parent && x.id !== task.id).sort((a, b) => a.pos - b.pos);
      const prev = sibs.filter(s => s.pos < task.pos).pop();
      if (!prev) return;
      task.parent = prev.id; task.project = prev.project;
      const k = childrenOf(prev.id).filter(x => x.id !== task.id);
      task.pos = (k.length ? Math.max(...k.map(x => x.pos)) : prev.pos) + 10;
      refresh();
    },
    outdent: (task) => {
      if (!task.parent) return;
      const p = parentOf(task); if (!p) return;
      task.parent = p.parent; task.pos = p.pos + 5;
      refresh();
    },
    promote: (task) => { if (!task.parent) return; task.parent = null; task.pos = maxRootPos() + 1000; setToast('Agora é uma tarefa independente'); refresh(); },
    move: (dragId, targetId, zone) => {
      const drag = TASKS.find(x => x.id === dragId), target = TASKS.find(x => x.id === targetId);
      if (!drag || !target || dragId === targetId || isDescendant(targetId, dragId)) return;
      if (zone === 'child') {
        drag.parent = target.id; drag.project = target.project;
        const kids = childrenOf(target.id).filter(k => k.id !== dragId);
        drag.pos = (kids.length ? Math.max(...kids.map(k => k.pos)) : target.pos) + 10;
      } else {
        drag.parent = target.parent;
        const tp = parentOf(target); if (tp) drag.project = tp.project;
        const sibs = TASKS.filter(t => t.parent === target.parent && t.id !== drag.id).sort((a, b) => a.pos - b.pos);
        const ti = sibs.findIndex(s => s.id === targetId);
        const before = zone === 'before' ? sibs[ti - 1] : target;
        const after = zone === 'before' ? target : sibs[ti + 1];
        const lo = before ? before.pos : (after ? after.pos - 20 : 0);
        const hi = after ? after.pos : (before ? before.pos + 20 : lo + 20);
        drag.pos = (lo + hi) / 2;
      }
      refresh();
    },
  ...
  const onComplete = (task, done) => {
    ...
    if (done && tk.recur) {
      ...
      const cloneTree = (orig, newParent, isRoot) => {
        const { id, pos, ...rest } = orig;
        const copy = mk(Object.assign({}, rest, { done: false, today: false, startAt: null, parent: newParent, col: 'todo', due: isRoot ? nd : orig.due }));
        TASKS.push(copy);
        childrenOf(orig.id).forEach(c => cloneTree(c, copy.id, false));
        return copy;
      };
      cloneTree(tk, tk.parent, true);
  ...
  const saveTask = (data) => {
    const subs = data.__subs; delete data.__subs;
    const existing = TASKS.find(x => x.id === data.id);
    if (existing) { Object.assign(existing, data); setToast('Tarefa atualizada'); }
    else {
      const t = mk(data); TASKS.push(t);
      (subs || []).filter(s => s.title.trim()).forEach((s, i) => TASKS.push(mk({ title: s.title.trim(), done: !!s.done, prio: s.prio || 0, notes: s.notes || '', assignees: s.assignees || [], parent: t.id, project: t.project, col: t.col, pos: t.pos + (i + 1) * 10 })));
      setToast('Tarefa criada');
    }
    setModal(null); refresh();
  };
  const deleteTask = (task) => { const ids = new Set([task.id, ...descendantsOf(task.id).map(d => d.id)]); for (let i = TASKS.length - 1; i >= 0; i--) if (ids.has(TASKS[i].id)) TASKS.splice(i, 1); setModal(null); setToast('Tarefa excluída'); refresh(); };
  ...
  const renderView = () => {
    const shared = { onComplete, onOpen: openTask, onCreate, onUpdate, onInlineSave, onToggleSub, onToast: setToast, treeApi };
    switch (route.view) {
  ...
  return (
    <div className="kg-app">
      ...
      <main className="kg-main">
        ...
        {route.view === 'calendar'
          ? <div className="kg-calhost"><CalendarPro variant={CAL_VARIANT[t.calVar] || 'agora'} /></div>
          : <div className="kg-scroll" ref={scrollRef}>{renderView()}</div>}
      </main>

      {/* ── Modais ── */}
      {modal && modal.kind === 'task' && <TaskModal task={modal.data} onSave={saveTask} onClose={() => setModal(null)} onDelete={deleteTask} api={treeApi} onOpen={openTask} />}
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Calendário" />
        <TweakRadio label="Estilo dos eventos" value={t.calVar} options={['Ágora', 'Google', 'Editorial']} onChange={v => setTweak('calVar', v)} />
      </TweaksPanel>
    </div>
  );
}
