...
- [~] T008 [US4] Criar `agents/kurisu/memory/exporters.py` com os exporters de **resumo** (read-only, UTC-3): `export_tarefas` (Kaguya, tarefas concluídas) e `export_financas` (Nami, gastos por período) — conforme [contracts/exporters.md](./contracts/exporters.md). **`export_tarefas` feito** (resumo datado: 1 doc/dia, `completed_at AT TIME ZONE 'America/Sao_Paulo'`); `export_financas` é **TODO**
...
- [x] T012 [US4] Criar `agents/kurisu/memory/sync.py` — orquestrador conforme [contracts/sync-job.md](./contracts/sync-job.md): export full → diff por URI + **hash-manifest** (`_state/{domain}_manifest.json`) → import novos, `delete_file`+reimport editados (hash difere), `delete_file` removidos com **trava ≤50%**, lotes de 25, watermark. ADIÇÃO/REMOÇÃO/**EDIÇÃO** ok. Falta só registrar os outros 7 exporters em `EXPORTERS` (depende de T008/T010)
...
- [x] T014 [P] [US4] Job agendado — **opção A**: serviço `kurisu-sync` no `docker-compose.yml` reusando a imagem do `web` (mesmo build `webapp/backend/Dockerfile`, sem Dockerfile próprio), com loop `python -m scripts.sync_kurisu_memory` + `sleep 86400` (padrão do `backup`, não é cron). Pré-requisito: `VERTEX_RAG_CORPUS_OPERACIONAL` no Dokploy
