...
- **Itens "no implementation details" desmarcados de propósito:** como na 027, as decisões técnicas
  já fechadas (corpus Vertex compartilhado, embeddings gerenciados, exporters por domínio, job de
  sync no container, trava ≤50%, fuso UTC-3, recência pós-recuperação) estão na seção *Restrições
  Técnicas*. O corpo de FR/US/SC permanece focado em capacidade.
- **Dependência dura:** a 028 depende da fundação da 027 (corpus Vertex + embeddings gerenciados +
  tool de busca). Não planejar/implementar a 028 antes da 027.
