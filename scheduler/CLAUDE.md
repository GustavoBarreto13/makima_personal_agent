...
| `kaguya_due_reminders` | A cada 5min | Lembrete **pontual** de tarefas vencendo (Kaguya): `list_tasks_due_for_reminder()` só traz o que ainda não foi avisado (trava em `tasks.due_reminder_sent_at`), envia via `send_notification()` e marca como notificado — cada tarefa avisa uma vez só, reagendar (`update_task` muda `due_date`/`due_time`) rearma; silencioso se nada venceu desde a última rodada (`scripts/send_kaguya_due_reminders.py`, spec 064 US5) |
