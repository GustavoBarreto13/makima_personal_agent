# AGENTS.md — makima_personal_agent
...
Cada agente especialista é um pacote local em `agents/`. Cada um tem seu próprio `AGENTS.md`:

| Agente | Domínio | Status | Documentação |
|---|---|---|---|
| `agents/nami/` | Finanças (PostgreSQL) | ✅ Fase 1 | `agents/nami/AGENTS.md` |
| `agents/kaguya/` | Tarefas + Agenda (PostgreSQL próprio + Calendar via MCP) + Calendar Hub (019) + Meu Dia (016) + Kanban views (024) + Tiny Experiments (029) + Metas (030) + Foco gameficado (037/062) | ✅ Fases 2, 011–020, 024–026, 029–030, 037, 062 | `agents/kaguya/AGENTS.md` |
| `agents/kurisu/` | Knowledge base (Vertex AI RAG — corpus ativo) + memória unificada | ✅ Fase 027 · 🔧 028 parcial | `agents/kurisu/AGENTS.md` |
| `agents/frieren/` | Livros (PostgreSQL + Google Books) | ✅ Fase 5a | `agents/frieren/AGENTS.md` |
| `agents/akane/` | Filmes (PostgreSQL + TMDB + Letterboxd) | ✅ Fase 015 | `agents/akane/AGENTS.md` |
| `agents/marin/` | Animes (PostgreSQL + Jikan/AniList + MAL OAuth) | ✅ Fase 021 | `agents/marin/AGENTS.md` |
| `agents/mai/` | Séries de TV (PostgreSQL + TMDB API v3) | ✅ Fase 022 | `agents/mai/AGENTS.md` |
| `agents/komi/` | Pessoas e contatos (PostgreSQL) | ✅ Fase 014 | `agents/komi/AGENTS.md` |
| `agents/journal/` | Diário (Violet) — **módulo de tools, não é sub-agente ADK**: sem `agent.py`, consumido só pelo router `/api/journal/*` do webapp | ✅ web | `agents/journal/AGENTS.md` |
| `agents/lucy/` | Email (Gmail) — agente somente leitura (IMAP) + digest matinal agendado | ✅ Fase 4 / 032 | `agents/lucy/AGENTS.md` |
...
Para detalhes do coordinator (infraestrutura, sessões, env vars, parse_mode), ver `coordinator/AGENTS.md`.
...
│   ├── Dockerfile
│   └── AGENTS.md        # infraestrutura, env vars, sessões, MCP Calendar, notas técnicas
├── agents/
│   ├── __init__.py
...
│   │   └── AGENTS.md    # tools, categorias, formatação, personalidade
...
│   │   └── AGENTS.md    # camada de lógica, tools, Calendar Hub, gcal_sync, personalidade
...
│   │   └── AGENTS.md    # arquitetura RAG, setup Vertex AI, checklist de ativação, Tutor, Conselho do Dia
│   ├── frieren/         # agente de livros — Fase 5a ✅
│   │   ├── __init__.py
│   │   ├── tools.py     # PostgreSQL + Google Books API
│   │   ├── agent.py     # frieren_agent
│   │   ├── schema_pg.sql # schema das tabelas PostgreSQL
│   │   └── AGENTS.md    # tools, schema PostgreSQL, menu interativo, personalidade
...
│   │   └── AGENTS.md    # tools, schema, cross-agent, personalidade
...
│   │   └── AGENTS.md    # tools, schema, TMDB/Letterboxd, personalidade
...
│   │   ├── schema_pg.sql # schema das 4 tabelas PostgreSQL
│   │   └── AGENTS.md    # tools, schema, MAL OAuth, personalidade
...
│   │   └── AGENTS.md    # tools, schema, digest, personalidade
...
│   │   ├── schema_pg.sql # schema das 4 tabelas PostgreSQL
│   │   └── AGENTS.md    # tools, schema, TMDB Bearer, personalidade
...
│       └── AGENTS.md    # tools, schema, integração com o webapp
├── mcp_servers/
│   ├── __init__.py
│   └── calendar/
│       ├── __init__.py
...
│   └── AGENTS.md        # o padrão + passo a passo "como adicionar um job novo"
├── scripts/
...
└── AGENTS.md            # este arquivo — visão geral e guia de navegação
```
...
| Árvore de arquivos detalhada | este `AGENTS.md` | README.md (só o mapa de topo) |
| Schema do banco (tabelas coluna a coluna) | `docs/referencia/POSTGRES.md` | AGENTS.md dos agentes (só nomes de tabelas) |
| Tools e regras de cada agente | `agents/<nome>/AGENTS.md` | tabela de agentes deste arquivo |
| Rotas da API do webapp | `webapp/docs/API.md` | webapp/AGENTS.md |
| Shells do frontend | `webapp/docs/FRONTEND.md` | webapp/frontend/src/pages/AGENTS.md |
...
3. Se criou/alterou tools de agente: atualizar o `agents/<nome>/AGENTS.md`
4. Se criou rotas ou telas: atualizar `webapp/docs/API.md` e/ou `webapp/docs/FRONTEND.md`
5. Se criou módulos novos: adicioná-los à árvore de arquivos deste `AGENTS.md`
...
6. Criar `agents/<dominio>/AGENTS.md` seguindo o padrão dos outros agentes
...
# variáveis mínimas necessárias (ver coordinator/AGENTS.md para lista completa):
...
at `specs/050-akane-letterboxd-carga/plan.md`.
<!-- SPECKIT END -->
